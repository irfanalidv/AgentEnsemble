...
def example_structured_output_pydantic():
    ...
    result = agent.run("Extract contact info from: John Doe, [email protected], (555) 123-4567")
...
def example_structured_output_tool_strategy():
    ...
    class ProductReview(BaseModel):
        ...
        rating: Optional[int] = Field(
            description="The rating of the product", ge=1, le=5, default=None
        )
    ...
    agent = StructuredAgent(
        name="review_analyzer", tools=[SearchTool()], response_format=response_format
    )
...
def example_structured_output_with_search():
    ...
    # Research topic
    result = agent.run("Research the latest developments in AI agents in 2024")
